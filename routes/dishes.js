...
router.get('/', async (req, res) => {
    try {
        ...
        const chooseLightImage = (img) => {
            ...
            // Нормализуем URL и вычислим имя файла
            const urlPath = img.startsWith('/') ? img : `/${img}`;
            const baseName = path.basename(urlPath);
            const dot = baseName.lastIndexOf('.');
            const nameOnly = dot > -1 ? baseName.substring(0, dot) : baseName;
            const thumbFs = path.join(uploadsDirFs, `${nameOnly}-100.webp`);
            const avifFs = path.join(uploadsDirFs, `${nameOnly}-400.avif`);
            const webpFs = path.join(uploadsDirFs, `${nameOnly}-400.webp`);
            const dirUrl = urlPath.substring(0, urlPath.lastIndexOf('/'));
            if (fs.existsSync(thumbFs)) return `${dirUrl}/${nameOnly}-100.webp`;
            if (fs.existsSync(avifFs)) return `${dirUrl}/${nameOnly}-400.avif`;
            if (fs.existsSync(webpFs)) return `${dirUrl}/${nameOnly}-400.webp`;
            return urlPath; // fallback на оригинал
        };
