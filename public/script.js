...
// Lazy image loader (singleton IntersectionObserver)
function hydrateLazyElement(target) {
    if (!target) return;
...
function setupLazyImages(root = document) {
    const pictures = root.querySelectorAll('picture.lazy-picture');
    ...
    if (observer) {
        ...
        pictures.forEach(hydrateLazyElement);
        images.forEach(hydrateLazyElement);
    }
}
...
async function loadData() {
    ...
    try {
        ...
        const [categoriesResponse, dishesResponse] = await Promise.all([
            fetch(`${API_BASE}/categories?restaurant=${isRestaurantMode()}`, { cache: 'no-cache' }),
            fetch(`${API_BASE}/dishes?restaurant=${isRestaurantMode()}&light=1`, { cache: 'no-cache' })
        ]);
...
function renderDishes() {
    ...
    function renderBatch() {
        ...
        for (let i = startIndex; i < end; i++) {
            ...
            const isPriority = i < 4; // только первые 4 карточки страницы
...
function createDishCard(dish, isPriority = false) {
    ...
    const webpUrl = isUnsplash ? `${imageUrl}${imageUrl.includes('?') ? '&' : '?'}fm=webp&auto=format&_v=${cacheBust}` : '';
    ...
    if (isLocal) {
        ...
        // Примечание: не используем <picture> для локальных картинок,
        // чтобы избежать пустых превью, если превью-файлы еще не сгенерированы.
        // Будем загружать оригинал с cache-busting параметром.
    }
    ...
    card.innerHTML = `
         <div class="card-image-container">
            ...
            <picture class="lazy-picture">
                <source data-srcset="${webpUrl}" type="image/webp" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 25vw">
                <img data-src="${imageUrl}" src="/ELEMENTS/image 2.png" alt="${dish.name[currentLanguage]}" loading="${isPriority ? 'eager' : 'lazy'}" decoding="async" fetchpriority="${isPriority ? 'high' : 'low'}" width="400" height="250" onerror="this.src='/ELEMENTS/image 2.png'">
            </picture>` : isLocal ? `
            <img data-src="${(normalizedLocal || imageUrl) + ((normalizedLocal || imageUrl).includes('?') ? '&' : '?') + 'v=' + cacheBust}" src="/ELEMENTS/image 2.png" alt="${dish.name[currentLanguage]}" loading="${isPriority ? 'eager' : 'lazy'}" decoding="async" fetchpriority="${isPriority ? 'high' : 'low'}" width="400" height="250" onerror="this.src='/ELEMENTS/image 2.png'">` : `
            <img data-src="${imageUrl}${imageUrl.includes('?') ? '&' : '?'}v=${cacheBust}" src="/ELEMENTS/image 2.png" alt="${dish.name[currentLanguage]}" loading="${isPriority ? 'eager' : 'lazy'}" decoding="async" fetchpriority="${isPriority ? 'high' : 'low'}" width="400" height="250" onerror="this.src='/ELEMENTS/image 2.png'">`
            }
            ${alcoholicOverlay}
         </div>
         <div class="card-body">
